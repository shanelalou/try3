			...
			$('#grid').flexigrid({
				...
				buttons : [
					...
					{name: 'PRINT', bclass: 'add', onpress: function(){
						window.open('print.php?student=<?php echo $_GET['student']?>&course=<?php echo $_GET['course'] ?>&year=<?php echo $_GET['year'] ?>&block=<?php echo $_GET['block'] ?>','','status=1,scrollbars=1, width=900,height=500');
					}},
					{separator:true},
				],
				colModel : [
					{display: 'CLASS CODE', name : 'col', width : 80, align: 'center'},
					{display: 'SUBJECT CODE', name : 'col', width : 90, align: 'center'},
					...
					{display: 'DAY', name : 'col', width : 50, align: 'center'},
					{display: 'TIME', name : 'col', width : 120, align: 'center'},
					{display: 'ROOM', name : 'col', width : 80, align: 'center'},
				],
				searchitems : [
				...
				title: 'SCHEDULE : <span><?php echo strtoupper(student($_GET['student'],'lastname').', '.student($_GET['student'],'firstname').' '.student($_GET['student'],'middlename')) ?></span> - <span><?php echo $_GET['course'].' '.$_GET['year'].'-'.$_GET['block'] ?></span>',
				height: windowHeight
			});
